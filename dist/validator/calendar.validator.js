 ...
 */
export function validateCreateCalendar(req, res, next) {
    ...
    if (colorCode !== undefined) {
        ...
        if (!/^#[0-9A-F]{6}$/i.test(colorCode)) {
            throw new ValidationError("Color code must be in hex format (e.g., #039BE5)");
        }
    }
 ...
 */
export function validateUpdateCalendar(req, res, next) {
    const { name, colorCode, isPrimary } = req.body;
    if (name !== undefined) {
