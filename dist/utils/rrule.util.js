...
export function buildRRuleString(options) {
    ...
    if (options.untilDate) {
        const until = options.untilDate.toISOString().replace(/[-:]/g, "").split(".")[0] + "Z";
...
export function parseRRuleString(rrule) {
    try {
        ...
        return options.frequency ? options : null;
    }
    catch {
        return null;
    }
 ...
 */
export function calculateOccurrences(startDate, endDate, rangeStart, rangeEnd, recurrenceRule, maxOccurrences = 100) {
    const occurrences = [];
 ...
 */
export function isOccurrenceDate(date, startDate, recurrenceRule) {
    // Đơn giản hóa: tạo list occurrences và check
    const occurrences = calculateOccurrences(startDate, new Date(startDate.getTime() + 3600000), // +1 hour
    new Date(date.getFullYear(), date.getMonth(), date.getDate()), new Date(date.getFullYear(), date.getMonth(), date.getDate(), 23, 59, 59), recurrenceRule, 10);
    return occurrences.some((occ) => occ.getFullYear() === date.getFullYear() &&
