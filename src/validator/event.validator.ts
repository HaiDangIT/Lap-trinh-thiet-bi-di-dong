    ...
    attendees.forEach((attendee: any, index: number) => {
      ...
      if (!isValidEmail(attendee.email)) {
        throw new ValidationError(
          `Attendee ${index + 1}: Invalid email format`
        );
      }
    });
  }
  ...
  if (reminders !== undefined) {
    ...
    reminders.forEach((reminder: any, index: number) => {
      if (
        ...
        throw new ValidationError(
          `Reminder ${index + 1}: minutesBefore is required`
        );
      }
      if (reminder.minutesBefore < 0) {
        throw new ValidationError(
          `Reminder ${index + 1}: minutesBefore must be positive`
        );
      }
    });
  }
 ...
 */
function validateRecurrenceRule(rule: any) {
 ...
 */
export function validateGetEventsInRange(
  req: Request,
  res: Response,
  next: NextFunction
) {
