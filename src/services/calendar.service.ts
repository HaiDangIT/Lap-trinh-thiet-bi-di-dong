...
export class CalendarService {
  ...
  async createCalendar(
    ...
    const existingPrimary = await calendarRepository.findPrimaryByUserId(
      userId
    );
    ...
    if (calendar.isPrimary) {
      ...
      if (calendarsCount > 1) {
        throw new ValidationError(
          "Cannot delete primary calendar. Please set another calendar as primary first."
        );
      }
    }
   ...
   */
  async setPrimaryCalendar(id: string, userId: string): Promise<Calendar> {
    const calendar = await calendarRepository.findById(id);
    if (!calendar) {
      throw new NotFoundError("Calendar not found");
    }
