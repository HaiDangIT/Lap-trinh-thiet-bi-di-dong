...
export async function setup2FA(req, res, next) {
    try {
        ...
        return res
            .status(200)
            .json(successResponse(result, "2FA setup initiated. Please scan the QR code and verify with a token to enable."));
    }
    catch (error) {
        next(error);
    }
...
export async function regenerateBackupCodes(req, res, next) {
    try {
        ...
        return res
            .status(200)
            .json(successResponse({ backupCodes }, "Backup codes regenerated successfully. Save these codes in a safe place!"));
    }
    catch (error) {
        next(error);
    }
