...
export function detectEventCategory(
  ...
  // Health patterns
  if (
    /doctor|dentist|hospital|bác sĩ|khám|health|medical|checkup/i.test(text)
  ) {
    return {
      category: "Health",
      icon: "🏥",
...
export function suggestBestTime(
  ...
  if (preferences?.avoidLunchTime) {
    filteredSlots = freeSlots.filter(
      (slot) => slot.start.getHours() < 11 || slot.start.getHours() >= 14
    );
  }
 ...
 */
export function suggestEventDuration(
  title: string,
  description?: string
): {
  duration: number;
 ...
 */
export function generateEventStats(
  events: Array<{
    title: string;
    description?: string;
    start: Date;
